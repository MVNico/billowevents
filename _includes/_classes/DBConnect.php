...
class DBConnect {
	private	$_tables,
			$_arrquery = array('config'=>array(),'columninfo'=>array(),'union'=>array()),
	...
	public function __construct($host,$user,$password,$database,$language='de',$charset = 'utf8') {
		...
		if (class_exists('CacheHandler')) {
			$this->_cache = new CacheHandler(array('caching_path'=>FLD_CACHE.'/','priority_required'=>0));
		}
	...
	public function setsubquerycolumn($subquerycolumn){
		$this->_arrquery['subquerycolumn']=subquerycolumn;
		return $this;
	}
	public function getsubquerycolumn(){
		return isset($this->_arrquery['subquerycolumn'])?$this->_arrquery['subquerycolumn']:NULL;
	}
	public function addsubquerycolumn($subquerycolumn){
		if(!isset($this->_arrquery['subquerycolumn'])){
			$this->_arrquery['subquerycolumn'] = array();
		}
	//	Helpers::xDebug($subquerycolumn);
		if(isset($subquerycolumn['split'])){
			$this->_arrquery['actions']['split'][$subquerycolumn['name']]=$subquerycolumn['split'];
		}
	//	Helpers::xDebug($this->_arrquery['action']);
		$this->_arrquery['subquerycolumn'][$subquerycolumn['name']]=$subquerycolumn;
		return $this;
	}
	...
	/**
	 * Initialisiert ein Seitenz�hler array
	 ...
	 * Rückgabezeile inklusive db_info
	 * @param string $query
	 ...
	 * @return array[result] array[db_info]
	 */
	private function select_keyvalue($query,$retkey,$options) {
		$this->Debug($query);
		$res = $this->_dbconnect->query($query);
		$this->sqlerror($res, $query);
		$ret['result'] = array();
		...
		for ($x=0;$x<$res->num_rows;$x++) {
			...
			if($retkey AND isset($row[$retkey])){
				$keyname=$row[$retkey];
				if(!isset($options['holdkey'])){unset($row[$retkey]);}
			}
			foreach ($row as $k => $v) {
				if ($this->is_serialized($v)){
					$row[$k] = unserialize($v);
	...
	/**
	 * Simuliert Anfrage an Datenbank mit Rückgabe eines<br>
	 * Arrays geteilt in Tabellennamen
	 * @param string $query
	 * @return array[result] array[db_info]
	 */
				...
				switch ($action) {
					case 'split':
									foreach ($fields as $k => $v) {
										if (isset($row[$k])) {
											if (!empty($row[$k]) OR $row[$k]==0) {
												$row[$k] = explode($v, $row[$k]);
											} else {
												$row[$k] = array();
									}}}
									break;
					default:		if (function_exists($action)) {
										foreach ($fields as $k => $v) {
											if (isset($row[$k])) {
												if (!empty($row[$k])) {
													$row[$k] = $action($row[$k]);
	...
	private function select_build($arrWerte) {
		...
		if(isset($this->_arrquery['subquery'])){
			$column = '*';
			$subquerycolumn = $this->getsubquerycolumn();
			if($subquerycolumn){
				$subqueryarr = array();
				foreach ($subquerycolumn as $k => $v){
					if (isset($v['command'])) {
						array_push($subqueryarr, $v['command'].' AS '.$v['name']);
					}else{
						array_push($subqueryarr, $v['name']);
					}
				}
				
				
				$column = implode(',', $subqueryarr);
			}
			array_push($arrquery, $column.' FROM ('.$this->_arrquery['subquery'].') AS r');
	...
	public function union(){
		$union = $this->_arrquery['union'];
		array_push($union, $this->select_build($this->_arrquery));
		$this->cleanQuery();
		$this->_arrquery['union'] = $union;
		return $this;
	}
	 ...
	 * 
	 */
	public function select($ReturnFormat="array",$Returnkey=false,$options=array()) {
		...
		if(!empty($this->_arrquery['union'])){
			$query = implode(' UNION ', $this->_arrquery['union']).' UNION '.$query;
		}
		...
		switch ($ReturnFormat) {
			...
			case 'keyvalue':	return $this->select_keyvalue($query, $Returnkey,$options);
								break;
	...
	public function insert($ArrayInsert,$ReturnFormat=NULL,$multiinsert=FALSE) {
		...
		switch ($ReturnFormat) {
			case "insert_id":	$this->Debug($StrQuery);
								array_push($this->_lastquery, $StrQuery);
	...
	/**
	 * Direkte SQL Anfrage mit vordefinierter Rückgabe
	...
	public function cleanQuery() {
		$this->_arrquery = array('config'=>array(),'columninfo'=>array(),'union'=>array());
		return $this;
	}
	...
	/**
	 * Prüft einen String und erzeugt daraus<br>
	 * ein array
	 * @param mix $var
	 */
	protected function setarray($var) {
		if (!is_array($var)) {
			$var = explode(',', $var);
		}
		foreach ($var as $k => $v) {
	...
	/**
	 * Prüft einen String ob er serializiert<br>
	 * wurde
	 * @param string $string
	 * @return bool
	 */
	 ...
	 * <b>Für das Array sind folgende Knotenpunkte nötig:</b><br>
	 * $table = array(<br>
	 ...
	 * &nbsp;&nbsp;&nbsp;&nbsp;'len' =&gt; String,Int Länge von Feld oder bei Enum zB. "'_self','_blank'"<br>
	...
	/**
	 * Anfrage der letzten $_lastquery_count SQL Statements
	 * @param int|array $id
	 */
	public function getlastquery($id=NULL){
		$ret=NULL;
		if (!is_null($id)) {
			if (is_array($id)) {
				foreach ($id as $v) {
					if (isset($this->_lastquery[$v])) {
						$ret[$v] = $this->_lastquery[$v];
					}
				}
			}else{
				if (isset($this->_lastquery[$id])) {
					$ret = $this->_lastquery[$id];
				}
			}
		}else{
			$ret = $this->_lastquery;
