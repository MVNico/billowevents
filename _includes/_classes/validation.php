    ...
    */
    ...
    public function run()
    {
        ...
        foreach( new ArrayIterator($this->validation_rules) as $var=>$opt)
        {
            ...
            if( array_key_exists('special_chars', $opt) && $opt['special_chars'] == true )
            {	
            	if(filter_var($this->source[$var], FILTER_VALIDATE_REGEXP, array("options"=>array("regexp"=>"/[^\da-zA-Z\.\_]/")))!== false)
            	{
            		$this->errors[$var] = $var . ' is not a valid. Special chars are not allowed.';
            	}
            }
            ...
            switch($opt['type'])
            {
                ...
                case 'password':
                	...
                	if(!array_key_exists($var, $this->errors))
                	{
                		$this->sanitizeString(htmlentities($var));
                	}
               		break;
     ...
     *
     */
    public function addRule($varname, $type, $required=false, $min=0, $max=0, $trim=false, $tagcheck=false)
    {
        $this->validation_rules[$varname] = array('type'=>$type, 'required'=>$required, 'min'=>$min, 'max'=>$max, 'trim'=>$trim, 'special_chars'=>$special_chars);
        /*** allow chaining ***/
        return $this;
    }
    ...
    private function validateSpecChars($var, $required=false)
    {
        if($required==false && strlen($this->source[$var]) == 0)
        {
            return true;
        }
        if(filter_var($this->source[$var], FILTER_VALIDATE_REGEXP, '[a-zA-Z0-9\.\_]\g') != FALSE)
        {
            $this->errors[$var] = $var . ' is not a valid. Special chars are not allowed.';
        }
    }
    
    public function sanitizeSpecChars($var)
    {
    	$this->sanitized[$var] = (string) filter_var($special_chars, FILTER_SANITIZE_SPECIAL_CHARS);
    }
